...
Travel planning for long-distance couples. Browser SPA + Supabase, free tier only.
...
2. **No API keys in the browser bundle.** Third-party calls go through Edge Functions.
...
8. **The app works with AI disabled.** Test with `VITE_ENABLE_AI=false` first.
